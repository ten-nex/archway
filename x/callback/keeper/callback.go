...
func (k Keeper) GetCallbacksByHeight(ctx sdk.Context, height int64, page, limit int) (callbacks []*types.Callback, err error) {
	...
	err = k.Callbacks.Walk(ctx, rng, func(key collections.Triple[int64, []byte, uint64], value types.Callback) (bool, error) {
		...
		return true, nil // Stop walking when limit is reached
	})
	return callbacks, err
}
